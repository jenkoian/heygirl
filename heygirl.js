...
(function (document) {
    ...
    var ratio, number, img;

    for (var i = 0; i < images.length; i++) {
        ratio = imageRatio(images[i]);
        number = Randomize(getGosling[ratio]());
        img = getGosling[ratio]()[number];
    ...
    for (var j = 0; j < pictures.length; j++) {
        ...
        ratio = imageRatio(picture_image[0]);
        number = Randomize(getGosling[ratio]());
        img = getGosling[ratio]()[number];

        for (var k = 0; k < srcs.length; k++) {
            srcs[k].srcset = img.imageurl
        }
    }
